#!/usr/bin/env node
/**
 * Check GDC 0 order prices in detail
 */
...
async function checkGDC0Prices() {
  console.log('🔍 Checking GDC 0 order prices...\n');

  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const supabase = createClient(supabaseUrl, supabaseKey);

  // Get all GDC 0 sales orders with items
  ...
  console.log('═══════════════════════════════════════════════════════════════════════');
  console.log('                        GDC 0 ORDERS - DETAILED PRICING                ');
  console.log('═══════════════════════════════════════════════════════════════════════\n');
  ...
  console.log('═══════════════════════════════════════════════════════════════════════');
  console.log(`  GDC 0 Summary: ${totalOrders} orders, Total Value: $${totalValue.toFixed(2)}`);
  console.log('═══════════════════════════════════════════════════════════════════════\n');
  ...
  const { data: zeroPrice } = await supabase
    ...
    .select(`
      ...
      unit_price
    `)
    .eq('sales_orders.order_series', 'GDC 0')
    .eq('unit_price', 0);
...
checkGDC0Prices().catch(console.error);
